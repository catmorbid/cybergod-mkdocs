...
**Example**
> Cyber Joe is handling a large caliber vehicle-mounted minigun and scores **8 hits** against a light vehicle, inflicting (after armor) **DC 7** or `1D8+1D6` damage per hit, which would mean making a [[damage-roll|Damage Roll]] of `8d8+8d6`. Instead of rolling 16 dice, GM calculates **Simplified damage:** $DC*Hits = 7*8 = 56$ damage. The Vehicle has Fatal Threshold of 40, so this is enough damage to disable the vehicle completely.