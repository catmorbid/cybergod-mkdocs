...
When prompted, the targets may attempt to resist mind manipulation by making a `Moderate(2) Psychology + Mind` Test. If they succeed, they resist the psionic effects.

The user can always apply [[psionics-potency|Potency Effect]] to increase the difficulty of target’s *Resistance Test* by +1 per effect stack.
...
- **Surface Scan ([[activate-standard|standard]], 1 [[energy]], 1 [[augment-effect-maintain|maintain]]):** Affects 1 → 2 → 4 → 8 **targets** in *Moderate* → *Long* → *Extreme* **Range**. You can hear what the targets are thinking and have the ability to project mental images to them.
- **Telepathic Link ([[activate-standard|standard]], 1 [[energy]], 1 [[augment-effect-maintain|maintain]])**: Form a mental link with a 1 → 2 → 4 → 8 consenting **targets**, enabling two-way communication. An unwilling target makes a [[#Resistance Test]]. Initial Link formation requires close proximity and physical contact, but afterwards, the Link can be maintained for a *Moderate* → *Long* duration without comprehensive limitations to distance.
- **Basic Illusion (([[activate-standard|standard]], 1 [[energy]], 1 [[augment-effect-maintain|maintain]])):** Create believable illusions affecting 1 → 2 → 4 → 8 **targets**, at *Moderate* → *Long* → *Extreme* **range** that are shattered upon close scrutiny. An illusion can be maintained for for *free* for a *Short* → *Moderate* **duration**. Illusions may include visual and audible components, but cannot simulate physical sensations. GM may call for a `Design + Mind` test to determine how impactful your illusion is – if necessary.

**Level 1 [[complications]]**
These complications are applied whenever a user activates a Level 1 Active Effect.

- **Low Disruption (1):** Spend 1 point of [[psionics-integrity|PSI Integrity]]
- **Mental Strain (1):** Take 1 point of [[stress|Stress]].
...
**Level 2 Complications**
These complications are applied whenever a user activates a Level 2 Active Effect.
...
These complications are applied whenever a user activates a Level 3 Active Effect.
